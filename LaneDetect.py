...
def mirror_line_to_right(left_line, frame_width):
    ...
    right_x1 = frame_width - distance_x1
...
def process_video(video_path, arrow):
    ...
    while cap.isOpened():
        ...
        if avg_left:
            ...
            left_lane_polygon = create_lane_polygon(avg_left, lane_width=150)
            # Draw left lane as transparent rectangle (green)
            result_frame = draw_transparent_polygon(result_frame, left_lane_polygon, (0, 255, 0), alpha=0.3)
        
        if avg_right:
            # Create lane polygons
            right_lane_polygon = create_lane_polygon(avg_right, lane_width=150)
            # Draw right lane as transparent rectangle (green)
            result_frame = draw_transparent_polygon(result_frame, right_lane_polygon, (0, 255, 0), alpha=0.3)
        ...
        # Display the result
        cv2.imshow('Lane Detection (Left Column & Mirrored)', result_frame)
